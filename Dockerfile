...
# Build Plex TV Station
COPY bootstrap /usr/local/sbin/bootstrap
COPY . /usr/src/plextvstation
RUN apt-get update
...
ENTRYPOINT ["/bin/dumb-init", "--", "/usr/local/sbin/bootstrap", "/usr/local/bin/plextvstation"]
